...
character(len=*), private, parameter   :: &
     version     = '$Id: lake_tile.F90,v 1.1.2.6 2012/06/19 18:34:54 pjp Exp $', &
...
subroutine read_lake_data_namelist(lake_n_lev)
  ...
  do 
     read (nml_unit, nml=lake_data_nml, iostat=io, end=10)
     if (check_nml_error (io, 'lake_data_nml')==0) exit ! from loop
  enddo
10  close (nml_unit)
...
! ============================================================================
function lake_cover_cold_start(land_mask, lonb, latb) result (lake_frac)
! creates and initializes a field of fractional lake coverage
  ...
  do k=1,n_dim_lake_types
    lake_frac(:,:,k) = lake_fraction(k)
  enddo

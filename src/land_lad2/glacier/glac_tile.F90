...
use land_tile_selectors_mod, only : tile_selector_type, SEL_GLAC, register_tile_selector
...
character(len=*), parameter   :: &
     version     = '$Id: glac_tile.F90,v 1.1.2.7 2012/06/19 18:34:54 pjp Exp $', &
...
subroutine read_glac_data_namelist(glac_n_lev, glac_dz)
  ...
  do 
     read (nml_unit, nml=glac_data_nml, iostat=io, end=10)
     if (check_nml_error (io, 'glac_data_nml')==0) exit ! from loop
  enddo
10   close (nml_unit)
  write (logunit, nml=glac_data_nml)
...
function glac_cover_cold_start(land_mask, lonb, latb) result (glac_frac)
  ...
  real,    intent(in) :: lonb(:,:), latb(:,:)! boundaries of the global grid cells
  ...
  do k=1,n_dim_glac_types
    glac_frac(:,:,k) = glacier_fraction(k)
  enddo

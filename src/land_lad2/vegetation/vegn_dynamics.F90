...
subroutine vegn_dynamics_init(id_lon, id_lat, time, delta_time)
  ...
  id_fast_soil_C = register_tiled_diag_field ( trim(module_name), 'fsc',   'fast soil carbon', 'kg C/m2')
  id_slow_soil_C = register_tiled_diag_field ( trim(module_name), 'ssc',   'slow soil carbon', 'kg C/m2')
...
subroutine biomass_allocation_ppa(cc)
  ...
  !cc%br_max = cc%bl_max / sp%LMA * sp%phiRL / sp%SRA ! equivalent.  LMA and SRA have inverse units
  ...
  if (cc%status == LEAF_ON) then
     ...
     BSWmax   = sp%alphaBM * (cc%DBH**sp%thetaBM - DBHwd**sp%thetaBM)
   	 ...
   	 if (hydraulics_repair .eq. .false.) then
    ...
    ! Gets radial depth of sapwood, and equates this depth to an effective age 
    tao_sapwood = (cc%dbh/2 - DBHwd/2) / (cc%dbh/2)/(cc%age) ! time
    
    ! compute running average of stress given time const of tissue
    !cc%astress_r = (1. - sp%alpha(CMPT_ROOT) * dt_fast_yr)*cc%astress_r + sp%alpha(CMPT_ROOT)*dt_fast_yr*cc%istress_r
    !cc%astress_x = (1. -    (1./tao_sapwood) * dt_fast_yr)*cc%astress_x +    (1./tao_sapwood)*dt_fast_yr*cc%istress_x
    !cc%astress_l = (1. - sp%alpha(CMPT_LEAF) * dt_fast_yr)*cc%astress_l + sp%alpha(CMPT_LEAF)*dt_fast_yr*cc%istress_l
...
!    cc%BM_ys       = cc%treeBM
	...
	! assign cohort id
	vegn%ccidMax = vegn%ccidMax + 1	! Adam Wolf
	cc%ccid = vegn%ccidMax
...
subroutine merge_cohorts(c1,c2)
  ...
  ! keep cohort id for larger cohort, defined by total areal biomass
  real :: size1, size2 ! size of each cohort
  size1 = c1%nindivs*c1%bliving
  size2 = c2%nindivs*c2%bliving
...
subroutine relayer_cohorts (vegn)
  ...
  do 
     ...
     if (assignNewCCID) then
	    ! assign cohort id
		vegn%ccidMax = vegn%ccidMax + 1	! Adam Wolf
		new(i)%ccid = vegn%ccidMax     
     endif

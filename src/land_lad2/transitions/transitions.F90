...
use utilities_mod, only : logunit, get_unit, error_mesg, FATAL, WARNING, NOTE, check_nml_error
...
character(len=*), parameter   :: &
     version = '$Id: transitions.F90,v 1.1.2.9 2012/06/19 18:34:54 pjp Exp $', &
...
character(len=64) :: fieldnames(N_LU_TYPES,N_LU_TYPES)
integer :: diag_id

! ---- namelist variables ---------------------------------------------------
logical :: do_landuse_change = .FALSE. ! if true, then the landuse changes with time
character(len=512) :: input_file = 'INPUT/lu.1700-2005.nc'
...
subroutine land_transitions_init(id_lon, id_lat)
 ...
 !call horiz_interp_init
  ...
  do 
     read (nml_unit, nml=landuse_nml, iostat=io, end=10)
     if (check_nml_error (io, 'landuse_nml')==0) exit ! from loop
  enddo
10  close (nml_unit)
  
  write (logunit, nml=landuse_nml)

  ! read restart file, if any
  inquire (file='INPUT/landuse.res', exist=fexist)
  if (fexist) then
     call error_mesg('land_transitions_init', 'reading restart "INPUT/landuse.res"', NOTE)
     unit = get_unit()
     open(unit, file='INPUT/landuse.res', form='formatted', action='read')
  ...
  do k2 = 1,N_LU_TYPES
     ...
     fieldnames(k1,k2)= trim(landuse_name(k1))//'_to_'//trim(landuse_name(k2))
  enddo
  enddo

  diag_id = get_unit()
  filename = 'DIAGNOSTICS/'//trim(module_name)//'_'//trim(diag_mod_name)
  open(unit=diag_id, file=trim(filename), form='formatted', action='write', position='rewind')
  write(diag_id,'(a)') 'units=1/year'
...
subroutine land_transitions_end()
 ...
 !if (do_landuse_change)  call horiz_interp_del(interp)
  if(allocated(time_in)) deallocate(time_in,buffer_in)
...
subroutine save_land_transitions_restart(timestamp)
  ...
  unit = get_unit()
  open(unit, file='RESTART/'//trim(timestamp)//'landuse.res', form='formatted', action='write')
...
subroutine split_changing_tile_parts(d_list,d_kind,a_kind,dfrac,a_list)
  ...
  character(len=32) :: ch_string, string1, string2
  ...
  if(overshoot_opt /= OPT_IGNORE.and.dfrac>area+overshoot_tolerance) then
     ...
     write(ch_string,'(a,i3,a,i3,a,i3,a)') '(',i,',',j,', face=',face,')'
     write(string1,*) dfrac
     write(string2,*) area
     call error_mesg('she_landuse',&
          'transition at '//trim(ch_string)// &
     !    'transition at ('//trim(string(i))//','//trim(string(j))//&
     !    ',face='//trim(string(face))//&
          ') from "'//trim(landuse_name(d_kind))// &
          '" to "'  //trim(landuse_name(a_kind))//&
          '" ('//trim(string1)//') is larger than area of "'&
          //trim(landuse_name(d_kind))//'" ('//trim(string2)//')', &
...
subroutine get_transitions(time0,time1,k1,k2,tran)
  ...
  ! ---- local vars
  integer :: i,j,k,sec,days,year, month, day, hour, minute, second
  ...
  call get_date(time1, year, month, day, hour, minute, second)
  write(diag_id,100) year, month, day, hour, minute, second, frac/part_of_year, trim(fieldnames(k1,k2))
100 format(i4,'-',i2.2,'-',i2.2,1x,i2.2,':',i2.2,':',i2.2,2x,1pe24.16,2x,a)

...
character(len=128) :: version='$Id: get_cal_time.F90,v 1.1.2.5 2011/03/28 16:24:15 pjp Exp $'
...
real    :: month_fraction
integer :: calendar_tm_i, calendar_in_i, io, nml_unit
...
if(.not.module_is_initialized) then
  ...
  do 
     read (nml_unit, nml=get_cal_time_nml, iostat=io, end=10)
     if (check_nml_error (io, 'get_cal_time_nml')==0) exit ! from loop
  enddo
10 close (nml_unit)
...
calendar_in_c = trim(cut0(calendar))

correct_form = (trim(calendar_in_c)) == 'noleap'     .or. (trim(calendar_in_c)) == 'NOLEAP'     .or. &
               (trim(calendar_in_c)) == '365_day'    .or. (trim(calendar_in_c)) == '365_DAY'    .or. &
               (trim(calendar_in_c)) == '360_day'    .or. (trim(calendar_in_c)) == '360_DAY'    .or. &
               (trim(calendar_in_c)) == 'julian'     .or. (trim(calendar_in_c)) == 'JULIAN'     .or. &
               (trim(calendar_in_c)) == 'no_calendar'.or. (trim(calendar_in_c)) == 'NO_CALENDAR'.or. &
               (trim(calendar_in_c)) == 'thirty_day_months' .or. (trim(calendar_in_c)) == 'THIRTY_DAY_MONTHS' .or. &
               (trim(calendar_in_c)) == 'gregorian'  .or. (trim(calendar_in_c)) == 'GREGORIAN'
...
if(.not.permit_conversion_local) then
  correct_form = (trim(calendar_in_c) == 'noleap'            .and. calendar_tm_i == NOLEAP)            .or. &
                 (trim(calendar_in_c) == 'NOLEAP'            .and. calendar_tm_i == NOLEAP)            .or. &
                 ...
                 (trim(calendar_in_c) == 'NO_CALENDAR'       .and. calendar_tm_i == NO_CALENDAR)       .or. &
                 (trim(calendar_in_c) == 'gregorian'         .and. calendar_tm_i == GREGORIAN)         .or. &
...
correct_form = trim(units(1:10)) == 'days since'    .or. &
               trim(units(1:11)) == 'hours since'   .or. &
               trim(units(1:13)) == 'minutes since' .or. &
               trim(units(1:13)) == 'seconds since'

if(calendar_in_i /= NO_CALENDAR) then
  correct_form = correct_form .or. &
               trim(units(1:11)) == 'years since'   .or. &
               trim(units(1:12)) == 'months since'
endif
...
else if(trim(units(1:12)) == 'months since') then

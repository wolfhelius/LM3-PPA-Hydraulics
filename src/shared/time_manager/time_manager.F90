...
use utilities_mod, only: error_mesg, FATAL, WARNING, logunit
...
character(len=128) :: version='$Id: time_manager.F90,v 1.1.2.7 2011/03/29 14:46:06 Peter.Phillipps Exp $'
...
!      Unit number for printed output. The default unit is 6.
!   </IN>
subroutine print_time (Time,str,unit)
type(time_type)  , intent(in) :: Time
character (len=*), intent(in), optional :: str
integer          , intent(in), optional :: unit
integer :: s,d,ticks, ns,nd,nt, outunit
...
10 format ('(a,i',i2.2,',a,i',i2.2,',a,i',i2.2,')')
  ...
  if (present(str)) then
     write (outunit,fmt) trim(str)//' day=', d, ', sec=', s, ', ticks=', ticks
  else
     write (outunit,fmt)       'TIME: day=', d, ', sec=', s, ', ticks=', ticks
  endif
...
! NOTE: there is no check for PE number
  ...
  mon = month_name(mo)
  if (present(str)) then
     write (outunit,10) trim(str)//' ', y,mon(1:3),' ',d,' ',h,':',m,':',s
  else
     write (outunit,10)       'DATE: ', y,mon(1:3),' ',d,' ',h,':',m,':',s
  endif
...
! </INFO>
 ...
 logical :: leap
 integer :: nr, icode, nml_unit, io, nn, outunit
...
logical :: test17=.true.,test18=.true.,test19=.true.
 ...
 nml_unit = get_unit()
 open (nml_unit, file='input.nml', form='formatted', action='read')
 rewind nml_unit
 read (nml_unit, nml=test_nml, iostat=io)
 close (nml_unit)
 outunit =  get_unit()
 open(outunit, file='test_time_manager.out', form='formatted', action='write')
...
! Tests of decrement_date and increment_date
 ...
 !  This test loops through every day of an 400 year period and writes a line to the output file for each day.
  ...
  if(test17) then
    ...
    write(outunit,'(/,a)') ' ====================================================='
    write(outunit,'(a)')   '  Warning: test17 produces voluminous output.'
    write(outunit,'(a)')   '  It can be turned off with: &test_nml test17=.false./'
    write(outunit,'(a,/)') ' ====================================================='
    call set_calendar_type(GREGORIAN)
    do year=1801,2200
      leap = mod(year,4) == 0

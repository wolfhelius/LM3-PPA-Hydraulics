...
!-----------------------------------------------------------------------
   ...
   integer :: yrmod, momod, dymod, nml_unit
   logical :: mod_leapyear

   character(len=128) :: version='$Id: time_interp.F90,v 1.1.2.5 2011/03/29 14:48:03 Peter.Phillipps Exp $'
...
contains
 ...
 subroutine time_interp_init()
   ...
   do 
      read (nml_unit, nml=time_interp_nml, iostat=io, end=10)
      if (check_nml_error (io, 'time_interp_nml')==0) exit ! from loop
   enddo
10 close (nml_unit)
...
logical, intent(in), optional :: correct_leap_year_inconsistency
  ...
  if (Time_end<=Timelist(1).or.Time_beg>=Timelist(n)) then
     ...
     write(outunit,*)'where n = size(Timelist) =',n
...
! </INFO>
 ...
 use    utilities_mod, only: logunit, FATAL, mpp_error
 ...
 integer :: index1, index2, mo, yr, timelist_len, ntest, nline
 real :: weight

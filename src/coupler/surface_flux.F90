...
character(len=*), parameter :: version = '$Id: surface_flux.F90,v 1.1.2.7 2012/06/19 18:34:53 pjp Exp $'
character(len=*), parameter :: tagname = '$Name: no_fms_b_pjp $'
...
subroutine surface_flux_1d (                                &
     ...
     dhdt_atm,  dedq_atm,   dtaudu_atm, dtaudv_atm, dt )
!</PUBLICROUTINE>
...
subroutine surface_flux_0d (                                                 &
     ...
     dhdt_atm_0,  dedq_atm_0,   dtaudu_atm_0, dtaudv_atm_0, dt )
...
subroutine surface_flux_2d (                                           &
  ...
  do j = 1, size(t_atm,2)
     call surface_flux_1d (                                           &
          ...
          dhdt_atm(:,j),  dedq_atm(:,j),   dtaudu_atm(:,j), dtaudv_atm(:,j), dt )
  end do
...
! ---- local vars ----------------------------------------------------------
  ...
  do 
     read (nml_unit, nml=surface_flux_nml, iostat=io, end=10)
     if (check_nml_error (io, 'surface_flux_nml')==0) exit ! from loop
  enddo
10  close (nml_unit)
  ...
  write (logunit, nml=surface_flux_nml)

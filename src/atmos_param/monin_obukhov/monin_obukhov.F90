...
character(len=128) :: version = '$Id: monin_obukhov.F90,v 1.1.2.1.2.4 2011/03/28 16:24:13 pjp Exp $'
...
!------------------- read namelist input -------------------------------
  ...
  do 
     read (nml_unit, nml=monin_obukhov_nml, iostat=io, end=10)
     if (check_nml_error (io, 'monin_obukhov_nml')==0) exit ! from loop
  enddo
10 close (nml_unit)
